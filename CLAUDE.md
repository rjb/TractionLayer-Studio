...
- `WorkflowForm.tsx` bounds the request with a 10-minute `AbortController` timeout (n8n holds the connection open for the whole run) and shows "Workflow started…"/"Workflow completed!" status copy around the submit — project-wide, not per-workflow, same as the output contract above. A workflow with an empty `inputs` array (e.g. Linear Backlog Triage, STU-80) renders as a bare trigger button, no form fields.
